...
if (isset($_GET['kembali'])) {
	$a = $_GET;
	$exec = mysql_query("update peminjaman set status='Dikembalikan' where status='Dipinjam' and barcodebuku='" . $a["kembali"] . "'");
	$exec = mysql_query("update buku set status='Ada' where barcode='" . $a["kembali"] . "'");

	$exec = mysql_query("insert into pengembalian (barcodeanggota, barcodebuku,haruskembali, tglkembali,petugas,status,keterlambatan, idpinjam) 
	select barcodeanggota, barcodebuku, tglkembali, CURRENT_DATE(), 
	'" . $_SESSION["username"] . "','Kembali',if(tglkembali<CURRENT_DATE,'Terlambat','-') as Keterlambatan, idpinjam
	from peminjaman a
	where a.barcodebuku='" . $a["kembali"] . "'");

	$data["isError"] = "0";
	$data["Alert"] = "Pengembalian buku berhasil..";
	echo json_encode($data);
}
